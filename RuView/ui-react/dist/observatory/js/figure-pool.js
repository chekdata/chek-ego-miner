...
const MAX_FIGURES = 4;
const WEARER_WIRE_COLOR = new THREE.Color(0x46ebd5);
const WEARER_JOINT_COLOR = new THREE.Color(0xff8edc);
...
export class FigurePool {
  ...
  _createFigure() {
    ...
    const wearerRingMat = new THREE.MeshBasicMaterial({
      color: WEARER_WIRE_COLOR,
      transparent: true,
      opacity: 0,
      blending: THREE.AdditiveBlending,
      depthWrite: false,
    });
    const wearerRing = new THREE.Mesh(
      new THREE.TorusGeometry(0.42, 0.035, 10, 48),
      wearerRingMat
    );
    wearerRing.rotation.x = Math.PI / 2;
    wearerRing.position.y = 0.04;
    wearerRing.visible = false;
    group.add(wearerRing);

    const wearerBeacon = new THREE.PointLight(WEARER_WIRE_COLOR, 0, 2.2);
    wearerBeacon.position.y = 1.2;
    group.add(wearerBeacon);
    ...
    return {
      ...
      wearerRing, wearerRingMat, wearerBeacon,
      visible: false,
      prevPositions,
      velocities,
      _initialized: false,
  ...
  _applyFigureAccent(fig, person) {
    const isWearer = Boolean(person?.isWearer);
    if (fig._wearerActive === isWearer) {
      return;
    }

    const wireColor = isWearer ? WEARER_WIRE_COLOR : new THREE.Color(this._settings.wireColor);
    const jointColor = isWearer ? WEARER_JOINT_COLOR : new THREE.Color(this._settings.jointColor);

    for (let i = 0; i < fig.joints.length; i++) {
      const joint = fig.joints[i];
      const nextColor = i === 0 ? wireColor : jointColor;
      joint.material.color.copy(nextColor);
      joint.material.emissive.copy(nextColor);
      if (joint._haloMat) {
        joint._haloMat.color.copy(jointColor);
      }
      if (joint._glow) {
        joint._glow.color.copy(jointColor);
      }
    }
    ...
    for (const segment of fig.bodySegments) {
      segment.mat.color.copy(wireColor);
      segment.mat.emissive.copy(wireColor);
    }

    fig.auraMat.color.copy(isWearer ? WEARER_WIRE_COLOR : wireColor);
    fig.personLight.color.copy(isWearer ? WEARER_WIRE_COLOR : wireColor);
    fig.wearerRingMat.color.copy(WEARER_WIRE_COLOR);
    fig.wearerBeacon.color.copy(WEARER_WIRE_COLOR);
    fig._wearerActive = isWearer;
  }
  ...
  update(data, elapsed) {
    ...
    for (let f = 0; f < this._figures.length; f++) {
      ...
      if (f < persons.length && isPresent) {
        const p = persons[f];
        this._applyFigureAccent(fig, p);
        const rawKeypoints = Array.isArray(p?.raw_keypoints) ? p.raw_keypoints : null;
        const hasRawKeypoints = Array.isArray(rawKeypoints) && rawKeypoints.length >= 17;
        const kps = hasRawKeypoints
          ? rawKeypoints
          : this._poseSystem.generateKeypoints(p, elapsed, breathPulse);
        this.applyKeypoints(fig, kps, breathPulse, p.position || [0, 0, 0], elapsed, p.pose, hasRawKeypoints, p);
        fig.visible = true;
      } else {
        if (fig.visible) {
          this.hide(fig);
   ...
   */
  applyKeypoints(fig, kps, breathPulse, pos, elapsed = 0, pose = 'standing', useRawKeypoints = false, person = null) {
    ...
    for (const seg of fig.bodySegments) {
      if (seg.isHead) {
        ...
        seg.mat.opacity = useRawKeypoints ? 0.018 : 0.15;
      } else {
        ...
        if (jA && jB) {
          ...
          seg.mat.opacity = useRawKeypoints ? 0.015 : 0.12;
        }
      }
      seg.mat.emissiveIntensity = useRawKeypoints ? 0.02 : 0.1 + Math.abs(breathPulse) * 0.4;
    }
    ...
    fig.auraMat.opacity = useRawKeypoints
      ? Math.min(0.02, this._settings.aura * 0.18)
    ...
    fig.aura.scale.set(
      useRawKeypoints ? auraShape.scaleX * 0.82 : auraShape.scaleX,
      useRawKeypoints ? auraShape.scaleY * 0.92 : auraShape.scaleY,
      useRawKeypoints ? auraShape.scaleZ * 0.82 : auraShape.scaleZ
    );
    fig.wearerRing.visible = Boolean(person?.isWearer);
    fig.wearerRing.position.set(cx, 0.04, cz);
    fig.wearerRing.scale.setScalar(useRawKeypoints ? 0.9 : 1.05 + Math.abs(breathPulse) * 3.0);
    fig.wearerRingMat.opacity = person?.isWearer ? 0.42 : 0;
    fig.wearerBeacon.position.set(cx, hipY + 1.1, cz);
    fig.wearerBeacon.intensity = person?.isWearer ? 0.55 + Math.abs(breathPulse) * 2.4 : 0;

    // Person light
    fig.personLight.position.set(pos[0], 1.2, pos[2]);
    fig.personLight.intensity = useRawKeypoints
      ? this._settings.glow * (person?.isWearer ? 0.16 : 0.08)
      : this._settings.glow * (person?.isWearer ? 0.62 : 0.4);
  ...
  hide(fig) {
    ...
    fig.wearerRing.visible = false;
    fig.wearerRingMat.opacity = 0;
    fig.wearerBeacon.intensity = 0;
