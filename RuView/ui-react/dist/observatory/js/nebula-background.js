/**
 * Room Atmosphere Background — Brand-led dark gradient with subtle particles
 * Keeps the original RuView atmosphere, but shifts it to CHEK blue / pink / aqua.
 */
...
void main() {
  ...
  // Brand-led dark atmosphere with subtle color variation
  float n1 = fbm(dir * 2.5 + uTime * 0.008, uOctaves);
  float n2 = fbm(dir * 4.0 - uTime * 0.005, max(1.0, uOctaves - 1.0));

  // CHEK palette: black base with brand blue, pink and aqua haze
  vec3 deepBlack = vec3(0.01, 0.02, 0.05);
  vec3 royalBlue = vec3(0.06, 0.14, 0.36);
  vec3 aquaTint  = vec3(0.04, 0.18, 0.19);
  vec3 pinkGlow  = vec3(0.20, 0.10, 0.20);

  vec3 bg = mix(deepBlack, royalBlue, n1 * 0.48);
  bg = mix(bg, aquaTint, n2 * 0.18);
  bg = mix(bg, pinkGlow, n1 * n2 * 0.12);

  // Subtle top-down gradient (lighter ceiling)
  float upFactor = max(0.0, dir.y) * 0.1;
  bg += vec3(0.03, 0.06, 0.11) * upFactor;
