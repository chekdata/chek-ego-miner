...
//! Offset  Size  Field
//! ------  ----  -----
//! 0       4     Magic: 0xC5110001 (v1) or 0xC5110005 (v2)
//! 4       1     Node ID
...
//! 17      1     Noise floor (i8)
//! 18      2     Reserved
//! 20      8     source_time_ns (LE u64, v2 only)
//! 20/28   N*2   I/Q pairs (n_antennas * n_subcarriers * 2 bytes)
//! ```
//!
...
const ESP32_CSI_MAGIC_V1: u32 = 0xC5110001;
const ESP32_CSI_MAGIC_V2: u32 = 0xC5110005;

/// ADR-018 header sizes in bytes (before I/Q data).
...
impl Esp32CsiParser {
    ...
    pub fn parse_frame(data: &[u8]) -> Result<(CsiFrame, usize), ParseError> {
        ...
        let (header_size, header_version, source_time_ns) = match magic {
            ESP32_CSI_MAGIC_V1 => (HEADER_SIZE_V1, 1, None),
            ESP32_CSI_MAGIC_V2 => {
                ...
                let source_time_ns = (&data[20..28]).read_u64::<LittleEndian>().map_err(|_| {
                    ParseError::ByteError {
                        offset: 20,
                        message: "Failed to read source_time_ns".into(),
                    }
                })?;
                (HEADER_SIZE_V2, 2, Some(source_time_ns))
            }
            _ => {
                return Err(ParseError::InvalidMagic {
                    expected: ESP32_CSI_MAGIC_V2,
                    got: magic,
                });
            }
        };
            ...
            match Self::parse_frame(&data[offset..]) {
                ...
                Err(_) => {
                    ...
                    while offset + 4 <= data.len() {
                        ...
                        if candidate == ESP32_CSI_MAGIC_V1 || candidate == ESP32_CSI_MAGIC_V2 {
                            break;
                        }
                        offset += 1;
                    }
...
mod tests {
    ...
    fn build_test_frame(node_id: u8, n_antennas: u8, subcarrier_pairs: &[(i8, i8)]) -> Vec<u8> {
        ...
        // Magic (offset 0)
        buf.extend_from_slice(&ESP32_CSI_MAGIC_V1.to_le_bytes());
        // Node ID (offset 4)
        buf.push(node_id);
    ...
    fn test_parse_valid_frame() {
        ...
        assert_eq!(consumed, HEADER_SIZE_V1 + 56 * 2);
    ...
    fn test_multi_antenna_frame() {
        ...
        assert_eq!(consumed, HEADER_SIZE_V1 + 12 * 2);
    ...
    #[test]
    fn test_parse_v2_frame_with_source_time() {
        let mut frame = build_test_frame(2, 1, &[(1, 2), (3, 4), (5, 6), (7, 8)]);
        frame[0..4].copy_from_slice(&ESP32_CSI_MAGIC_V2.to_le_bytes());
        frame.splice(20..20, 1_234_567_890u64.to_le_bytes());

        let (parsed, consumed) = Esp32CsiParser::parse_frame(&frame).unwrap();

        assert_eq!(consumed, HEADER_SIZE_V2 + 8);
        assert_eq!(parsed.metadata.node_id, 2);
        assert_eq!(parsed.metadata.source_time_ns, Some(1_234_567_890));
        assert_eq!(parsed.metadata.header_version, 2);
    }
}
