...
pub struct CsiMetadata {
    ...
    pub sequence: u32,
    /// Device monotonic source timestamp in nanoseconds since boot, if present in the wire header.
    pub source_time_ns: Option<u64>,
    /// Raw CSI header version (`1` for 20-byte header, `2` for 28-byte header).
