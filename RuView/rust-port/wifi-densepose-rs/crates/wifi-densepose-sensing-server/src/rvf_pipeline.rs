...
const DEFAULT_SONA_LORA_ALPHA: f32 = 8.0;

fn sona_profile_delta_from_value(value: &serde_json::Value) -> Option<Vec<f32>> {
    let lora_a: Vec<f32> = serde_json::from_value(value.get("lora_a")?.clone()).ok()?;
    let lora_b: Vec<f32> = serde_json::from_value(value.get("lora_b")?.clone()).ok()?;
    ...
    let rank = value
        ...
        .unwrap_or(lora_b.len());
    if rank == 0 || lora_a.len() % rank != 0 || lora_b.len() % rank != 0 {
        return None;
    }

    let out_features = lora_b.len() / rank;
    if out_features != 1 {
        return None;
    }
    ...
    let alpha = value
        .get("alpha")
        .or_else(|| value.get("lora_alpha"))
        .and_then(|raw| raw.as_f64())
        .unwrap_or(DEFAULT_SONA_LORA_ALPHA as f64) as f32;
    let scale = alpha / rank as f32;
    let param_count = lora_a.len() / rank;
    let mut delta = vec![0.0f32; param_count];

    for idx in 0..param_count {
        let mut value = 0.0f32;
        for r in 0..rank {
            value += lora_a[idx * rank + r] * lora_b[r];
        }
        delta[idx] = value * scale;
    }

    Some(delta)
}

pub fn decode_sona_profile_deltas(reader: &RvfReader) -> BTreeMap<String, Vec<f32>> {
    let mut profiles = BTreeMap::new();
    for (header, payload) in reader.segments() {
        ...
        let Some(env) = value.get("env").and_then(|raw| raw.as_str()) else {
            continue;
        };
        let Some(delta) = sona_profile_delta_from_value(&value) else {
            continue;
        };
        profiles.insert(env.to_string(), delta);
    }
    profiles
}
...
mod tests {
    ...
    #[test]
    fn decode_sona_profile_deltas_reconstructs_flat_delta() {
        let mut b = RvfModelBuilder::new("sona-delta", "1.0.0");
        b.set_weights(&[1.0, 2.0, 3.0, 4.0]);
        b.add_sona_profile("scene-a-geometry", &[0.1, 0.0, -0.1, 0.2], &[0.5]);

        let data = b.build().unwrap();
        let reader = RvfReader::from_bytes(&data).unwrap();
        let deltas = decode_sona_profile_deltas(&reader);

        let delta = deltas.get("scene-a-geometry").expect("decoded profile");
        assert_eq!(delta.len(), 4);
        assert!((delta[0] - 0.4).abs() < 1e-6);
        assert!((delta[1] - 0.0).abs() < 1e-6);
        assert!((delta[2] + 0.4).abs() < 1e-6);
        assert!((delta[3] - 0.8).abs() < 1e-6);
    }
