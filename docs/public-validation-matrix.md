...
- `template-supported`: evidence template is documented, but public evidence still needs to be collected
...
| Public E2E summary | `supported` | `./cli/chek-ego-miner public-e2e --tier lite --json` | Host OS, tier, camera readiness, VLM policy, local capture result and upload policy are explicit |
...
| --- | --- | --- |
| First contributor | `template-supported` | clean machine, tier choice, host check, camera smoke, first local bundle |
| Interrupted install resume | `template-supported` | rerun readiness, detect existing runtime, continue without reinstalling blindly |
| Agent-guided install | `supported` | assistant asks tier, OS, app state and camera hardware before commands |
| Returning user readiness | `template-supported` | compare current host/camera/VLM state with prior known-good setup |
...
See [Public and Private Runtime Boundary](./public-private-boundary.md) for the
rule that internal evidence may inform the roadmap but cannot replace public
